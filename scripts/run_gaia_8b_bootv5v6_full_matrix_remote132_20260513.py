...
CONFIGS = {
    "bootv5": ROOT / "configs/system_bootv5_graph_free.json",
    "bootv6": ROOT / "configs/system_bootv6_long_rules.json",
}
DEFAULT_CONFIG = CONFIGS["bootv5"]
...
PREFIX = os.environ.get(
    "GAIA_8B_BOOTV5V6_FULL_MATRIX_PREFIX",
    datetime.now().strftime("%Y%m%d_%H%M_8b_bootv5v6_fullmatrix_remote132"),
).strip()
MASTER_NAME = f"{PREFIX}_master"
MASTER_LOG = Path(
    os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_MASTER_LOG", QUEUE_LOG_ROOT / f"{MASTER_NAME}.log")
)
MANIFEST_JSON = QUEUE_LOG_ROOT / f"{PREFIX}_manifest.json"
SUMMARY_JSON = QUEUE_LOG_ROOT / f"{PREFIX}_summary.json"
REPORT_DOC = Path(
    os.environ.get(
        "GAIA_8B_BOOTV5V6_FULL_MATRIX_REPORT_DOC",
        DESIGN_ROOT / f"26.5.13_{PREFIX}_GAIA_8B_V5V6全矩阵执行记录.md",
    )
)

CONCURRENCY = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_CONCURRENCY", "20"))
POLL_SECONDS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_POLL_SECONDS", "120"))
CODEX_TIMEOUT_SECONDS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_CODEX_TIMEOUT_SECONDS", "3600"))
BOOT_WORKERS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_BOOT_WORKERS", "6"))
BOOTSTRAP_WORKERS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_BOOTSTRAP_WORKERS", "2"))
OFFLINE_WORKERS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_OFFLINE_WORKERS", "4"))
CRITIC_SHARD_CONCURRENCY = os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_CRITIC_SHARD_CONCURRENCY", "99").strip() or "99"
PARTIAL_SOURCE_MIN_STATES = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_PARTIAL_SOURCE_MIN_STATES", "80"))
PARTIAL_SOURCE_STALL_SECONDS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_PARTIAL_SOURCE_STALL_SECONDS", "3600"))
LONGTAIL_FRACTION = float(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_LONGTAIL_FRACTION", "0.95"))
LONGTAIL_MAX_MISSING = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_LONGTAIL_MAX_MISSING", "2"))
LONGTAIL_STALLED_SECONDS = int(os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_LONGTAIL_STALLED_SECONDS", "900"))

CODEX_BIN = os.environ.get(
    "NLRL_CODEX_CLI_PATH",
    "/data/xsy/.vscode-server/extensions/openai.chatgpt-26.429.30905-linux-x64/bin/linux-x86_64/codex",
)
CODEX_PROXY = os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_CODEX_PROXY", "http://127.0.0.1:17890")
BOOT_KEYS = ["bootv5", "bootv6"]
METHOD_ORDER = ["A_full", "B1", "B2", "B3"]
DOWNSTREAM_BOOT_KEYS = [
    key.strip()
    for key in os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_DOWNSTREAM_BOOT_KEYS", "bootv6").split(",")
    if key.strip()
]
...
def configure_modules() -> None:
    ...
    base.VLLM_GPU_MEMORY_UTILIZATION = os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_VLLM_GPU_MEMORY_UTILIZATION", "0.90")
    base.VLLM_MAX_NUM_SEQS = os.environ.get("GAIA_8B_BOOTV5V6_FULL_MATRIX_VLLM_MAX_NUM_SEQS", "48")
...
def append_report(summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "- 计数口径：BOOT-V5/BOOT-V6 各 3 个 fresh boot source；BOOT-V5 只保留 boot-only dev/test，"
        "BOOT-V6 继续一对一扇出 boot-only、A_full、B1、B2、B3 的 dev/test。",
        ...
        "- 先跑 BOOT-V5/BOOT-V6 的 3 repeat boot-only；boot-only 收口后只让 BOOT-V6 按 A_full、B1、B2、B3 分层推进。",
        "- 每个 repeat 绑定自己的 fresh boot source；BOOT-V6 后续 A_full/B1/B2/B3 只复用同 repeat 的 boot_dev source。",
        "- 长尾释放口径：达到 95% 或缺失不超过 2 个任务，且 15 分钟无新 state，可让后续任务补位。",
        "",
        "## 2026-05-13 白天验收计划",
        "",
        "- 检查 manifest、summary、活进程表和 8 个 endpoint，确认 36 条 counted eval 的完成/长尾释放/失败分布。",
        ...
        "- 产出方向选择：boot-only、A_full、B1、B2、B3 横向比较，给出 8B 下一轮主线和保留备线。",
        "",
        "## 当前分数快照",
        "",
...
def main() -> int:
    ...
    base.append_process_row(
        ...
        command=os.environ.get(
            "GAIA_8B_BOOTV5V6_FULL_MATRIX_MASTER_COMMAND",
            shell_join([str(PYTHON), "-u", str(Path(__file__).resolve())]),
        ),
        log_path=MASTER_LOG,
        notes=(
            f"8B BOOT-V5/V6 full matrix remote132; expected_counted_eval_count={EXPECTED_COUNTED_EVALS}; "
            f"boot_workers={BOOT_WORKERS}; offline_workers={OFFLINE_WORKERS}; summary={SUMMARY_JSON}; report={REPORT_DOC}"
        ),
    )
    scheduler = threading.Thread(target=scheduler_loop, name="gaia-8b-bootv5v6-fullmatrix-scheduler", daemon=True)
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max(1, OFFLINE_WORKERS), thread_name_prefix="8b-offline") as offline_pool:
            with ThreadPoolExecutor(max_workers=max(1, BOOT_WORKERS), thread_name_prefix="8b-boot") as boot_pool:
                ...
                while pending:
                    ...
                    for future in done_now:
                        ...
                        try:
                            boot_sources.append(future.result())
                        except Exception as exc:
            ...
            boot_sources.sort(key=lambda source: (int(source.boot_key.rsplit("_r", 1)[1]), source.boot_key))
            downstream_sources = [
                source for source in boot_sources if source.boot_key.rsplit("_r", 1)[0] in DOWNSTREAM_BOOT_KEYS
            ]
            skipped_sources = [source.boot_key for source in boot_sources if source not in downstream_sources]
            if skipped_sources:
                log(f"skip downstream for boot sources: {', '.join(skipped_sources)}")
                write_manifest("running", {"stage": "downstream_filter_applied", "skipped_sources": skipped_sources})
            for method_key in METHOD_ORDER:
                ...
                while pending_methods:
                    ...
                    log(
                        f"waiting offline {method_key}: "
                        f"done={len(method_futures) - len(pending_methods)}/{len(method_futures)}"
                    )
                    write_manifest("running", {"stage": f"waiting_offline_{method_key}"})
                    if pending_methods:
                        time.sleep(POLL_SECONDS)
