...
PREFIX = os.environ.get(
    "GAIA_BOOTV4_AFULL_STABILITY_PREFIX",
    datetime.now().strftime("%Y%m%d_%H%M_9b_bootv4_afull_stability2"),
).strip()
MASTER_NAME = f"{PREFIX}_master"
MASTER_LOG = Path(
    os.environ.get(
        "GAIA_BOOTV4_AFULL_STABILITY_MASTER_LOG",
...
REPEATS = int(os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_REPEATS", "2"))
CONCURRENCY = int(os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_CONCURRENCY", "20"))
POLL_SECONDS = int(os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_POLL_SECONDS", "180"))
CODEX_TIMEOUT_SECONDS = int(os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_CODEX_TIMEOUT_SECONDS", "3600"))
PREPROCESS_NOTE_CHAR_CAP = os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_PREPROCESS_NOTE_CHAR_CAP", "700")
PARTIAL_STOP_MIN_STATES = int(os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_PARTIAL_STOP_MIN_STATES", "80"))
PARTIAL_STOP_STALL_SECONDS = int(
    os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_PARTIAL_STOP_STALL_SECONDS", "3600")
)
...
def ensure_9b_services() -> None:
    mode = os.environ.get("GAIA_BOOTV4_AFULL_STABILITY_DEPLOY_MODE", "auto").strip().lower()
...
def launch_eval(
    *,
    ...
    prev.append_process_row(
        ...
        notes=(
            f"9B BOOT-V4 -> A_full stability repeat={repeat_index} {label}; "
...
def run_offline_a(repeat_index: int, source_run: Path, source_skill: Path) -> Path:
    source = prev.base.BootSource(
        model_key="9b",
        boot_key=f"bootv4_s{repeat_index}",
        label=f"9B BOOT-V4 stability repeat {repeat_index} A_full source",
        boot_dev_run=source_run,
        boot_skill=source_skill,
        supplemental_runs=[],
        source_notes=(
            f"2026-05-11 stability repeat {repeat_index}: A_full from same fresh BOOT-V4 "
...
def run_repeat(repeat_index: int) -> None:
    ...
    boot_dev = launch_eval(
        ...
        repeat_index=repeat_index,
        run_name=f"{PREFIX}_{repeat_tag}_9b_bootv4_boot_dev_c{CONCURRENCY}",
        dataset=DEV_DATASET,
    ...
    launch_eval(
        ...
        repeat_index=repeat_index,
        run_name=f"{PREFIX}_{repeat_tag}_9b_bootv4_boot_test_c{CONCURRENCY}",
    ...
    launch_eval(
        label=a_dev_label,
        repeat_index=repeat_index,
        run_name=f"{PREFIX}_{repeat_tag}_9b_bootv4_A_full_dev_eval_c{CONCURRENCY}",
        dataset=DEV_DATASET,
        lane=first_free_lane(0),
        skill_path=a_skill,
    )
    launch_eval(
        label=a_test_label,
        repeat_index=repeat_index,
        run_name=f"{PREFIX}_{repeat_tag}_9b_bootv4_A_full_test_eval_c{CONCURRENCY}",
        dataset=TEST_DATASET,
        lane=first_free_lane(1),
        skill_path=a_skill,
    )
...
def main() -> int:
    ...
    master_command = os.environ.get(
        "GAIA_BOOTV4_AFULL_STABILITY_MASTER_COMMAND",
    ...
    prev.append_process_row(
        ...
        notes=(
            f"9B BOOT-V4 -> A_full stability repeats={REPEATS}; "
