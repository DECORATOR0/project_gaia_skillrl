...
PREFIX = os.environ.get("GAIA_24EXP_PREFIX", "").strip() or datetime.now().strftime(
    "%Y%m%d_%H%M%S_8b9b_archv53_bootv3v4_24exp"
)
...
DO_REMOTE_CLEANUP = os.environ.get("GAIA_24EXP_CLEANUP_REMOTE132", "1").strip().lower() not in {
    "0",
    "false",
    "no",
    "off",
...
def run_offline_variant(source: BootSource, *, method_key: str, strategy: str, graph_policy: str) -> Path:
    ...
    os.environ["NLRL_CRITIC_SHARD_CONCURRENCY"] = "1"
...
def write_summary(status: str, *, start_time: str, end_time: str | None = None, analysis_text: str = "") -> dict[str, Any]:
    ...
    payload = {
        ...
        "matrix": {
            ...
            "expected_counted_eval_count": 24,
        ...
        "config_card": {
            ...
            "critic_shard_concurrency": 1,
            "strong_model_serial": True,
        },
...
def append_report(summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- counted evals: `{summary['matrix']['counted_eval_count']}/24`",
...
def run_final_codex_analysis(summary: dict[str, Any]) -> str:
    prompt = f"""You are reviewing a GAIA SkillRL experiment matrix.
...
- dependency anomalies, especially supplemental runs;
...
def main() -> int:
    ...
    QUEUE_LOG_ROOT.mkdir(parents=True, exist_ok=True)
    LAUNCH_LOG_ROOT.mkdir(parents=True, exist_ok=True)
    for path in (PYTHON, CONFIG, DEV_DATASET, TEST_DATASET, EXISTING_9B_BOOTV3_DEV, EXISTING_9B_BOOTV4_DEV):
        if not path.exists():
    ...
    append_process_row(
        ...
        log_path=QUEUE_LOG,
        notes=f"24 eval matrix; strong Codex calls serial; report={REPORT_DOC}; summary={SUMMARY_JSON}",
    )
    ...
    try:
        ...
        bootv3_8b_dev = f"{PREFIX}_8b_bootv3_boot_dev_c{TASK_CONCURRENCY}"
        enqueue_eval(
            EvalSpec(
                key="8b_bootv3_boot_dev",
                label="8B BOOT-V3 boot_dev",
                model_key="8b",
                dataset=DEV_DATASET,
                run_name=bootv3_8b_dev,
                bootstrap=True,
                preferred_lanes=["remote132_gpu2_8b"],
                notes="BOOT-V3 + ARCH5.3 fresh bootstrap/eval",
            )
        )
        bootv3_8b_skill = wait_for_bootstrap_skill(bootv3_8b_dev)
        bootv3_8b_run = latest_run_dir(bootv3_8b_dev)
        if bootv3_8b_run is None:
            raise RuntimeError("8B bootv3 run dir missing after skill ready")
        enqueue_eval(
            EvalSpec(
                key="8b_bootv3_boot_test",
                label="8B BOOT-V3 boot_test",
                model_key="8b",
                dataset=TEST_DATASET,
                run_name=f"{PREFIX}_8b_bootv3_boot_test_c{TASK_CONCURRENCY}",
                skill_path=bootv3_8b_skill,
                preferred_lanes=["remote132_gpu3_8b"],
                notes="uses 8B BOOT-V3 bootstrap skill",
            )
        )

        bootv4_8b_dev = f"{PREFIX}_8b_bootv4_boot_dev_c{TASK_CONCURRENCY}"
        enqueue_eval(
            EvalSpec(
                key="8b_bootv4_boot_dev",
                label="8B BOOT-V4 boot_dev",
                model_key="8b",
                dataset=DEV_DATASET,
                run_name=bootv4_8b_dev,
                bootstrap=True,
                bootstrap_preprocess=True,
                preferred_lanes=["remote132_gpu4_8b"],
                notes="BOOT-V4 preprocess + ARCH5.3 fresh bootstrap/eval",
            )
        )
        bootv4_8b_skill = wait_for_bootstrap_skill(bootv4_8b_dev)
        bootv4_8b_run = latest_run_dir(bootv4_8b_dev)
        if bootv4_8b_run is None:
            raise RuntimeError("8B bootv4 run dir missing after skill ready")
        enqueue_eval(
            EvalSpec(
                key="8b_bootv4_boot_test",
                label="8B BOOT-V4 boot_test",
                model_key="8b",
                dataset=TEST_DATASET,
                run_name=f"{PREFIX}_8b_bootv4_boot_test_c{TASK_CONCURRENCY}",
                skill_path=bootv4_8b_skill,
                preferred_lanes=["remote132_gpu5_8b"],
                notes="uses 8B BOOT-V4 bootstrap skill",
            )
        )

        existing_v3_skill = EXISTING_9B_BOOTV3_DEV / "bootstrap/skill_after_bootstrap/gaia-general-skill/SKILL.md"
        existing_v4_skill = EXISTING_9B_BOOTV4_DEV / "bootstrap/skill_after_bootstrap/gaia-general-skill/SKILL.md"
        if not existing_v3_skill.exists() or not existing_v4_skill.exists():
            raise RuntimeError("existing 9B boot skill missing")

        v4_missing = source_missing_task_ids(EXISTING_9B_BOOTV4_DEV)
        v4_supplemental_runs: list[Path] = []
        if v4_missing:
            dep_run_name = f"{PREFIX}_9b_bootv4_dependency_fill_dev_missing{len(v4_missing)}_c{TASK_CONCURRENCY}"
            enqueue_eval(
                EvalSpec(
                    key="9b_bootv4_dependency_fill",
                    label="9B BOOT-V4 dependency fill dev",
                    model_key="9b",
                    dataset=DEV_DATASET,
                    run_name=dep_run_name,
                    counted=False,
                    skill_path=existing_v4_skill,
                    task_ids=v4_missing,
                    preferred_lanes=["remote132_gpu0_9b"],
                    notes=f"fills missing source states for B generation: {','.join(v4_missing)}",
                )
            )
            dep_record = wait_eval_done(dep_run_name)
            if dep_record["returncode"] != 0:
                raise RuntimeError(f"dependency fill failed: {dep_record}")
            dep_dir = latest_run_dir(dep_run_name)
            if dep_dir is None:
                raise RuntimeError("dependency fill run dir missing")
            v4_supplemental_runs.append(dep_dir)

        sources = [
            BootSource("9b", "bootv3", "9B BOOT-V3 ARCH5.3", EXISTING_9B_BOOTV3_DEV, existing_v3_skill),
            BootSource(
                "9b",
                "bootv4",
                ...
                existing_v4_skill,
                supplemental_runs=v4_supplemental_runs,
                source_notes=f"dependency_fill_missing={v4_missing}",
            ),
        ]
        methods_9b = [
            ("B1", "sharded", "locked"),
        ...
        for source in sources:
            for method_key, strategy, graph_policy in methods_9b:
                skill = run_offline_variant(source, method_key=method_key, strategy=strategy, graph_policy=graph_policy)
                for split, dataset in [("dev", DEV_DATASET), ("test", TEST_DATASET)]:
                    enqueue_eval(
                        EvalSpec(
                            ...
                            skill_path=skill,
                            preferred_lanes=["remote132_gpu0_9b", "remote132_gpu1_9b"],
                            notes=f"skill from {skill}",
                        )
                    )

        wait_eval_done(bootv3_8b_dev)
        wait_eval_done(bootv4_8b_dev)
        sources_8b = [
            BootSource("8b", "bootv3", "8B BOOT-V3 ARCH5.3", bootv3_8b_run, bootv3_8b_skill),
            BootSource("8b", "bootv4", "8B BOOT-V4 ARCH5.3 preprocess", bootv4_8b_run, bootv4_8b_skill),
        ]
        methods_8b = [
            ("B1", "sharded", "locked"),
            ("B2", "graph_b2", "graph_unlocked"),
        ]
        for source in sources_8b:
            for method_key, strategy, graph_policy in methods_8b:
                skill = run_offline_variant(source, method_key=method_key, strategy=strategy, graph_policy=graph_policy)
                for split, dataset in [("dev", DEV_DATASET), ("test", TEST_DATASET)]:
                    enqueue_eval(
                        EvalSpec(
        ...
        end_time = now()
        summary = write_summary("evals_finished_before_final_analysis", start_time=start_time, end_time=end_time)
        analysis = run_final_codex_analysis(summary)
        end_time = now()
        summary = write_summary("finished", start_time=start_time, end_time=end_time, analysis_text=analysis)
