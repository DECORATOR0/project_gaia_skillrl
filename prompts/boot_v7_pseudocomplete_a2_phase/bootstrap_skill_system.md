...
- Do not leave terminal no-`Next` phases; every phase must follow the V7 pseudo-complete graph contract below.

BOOT-V7 pseudo-complete graph contract:
- This version inherits the BOOT-V3 skill design style, then adds one hard routing constraint.
- The strong model should still design a compact set of useful GAIA work states from the runtime contract and batch snapshot.
...
- Put `version: boot-v7` and `parent: boot-v3` in metadata.
...
- Avoid making all tasks pass through every phase; the dense V7 edge set permits skipping, it does not require visiting each phase.
