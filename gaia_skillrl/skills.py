...
def _normalize_skill_markdown(
    ...
    *,
    ...
    inherited_meta = existing_meta or {}
    description = str(meta.get("description", "") or inherited_meta.get("description", "")).strip()
    if description:
        description = " ".join(part.strip() for part in description.splitlines() if part.strip())
    if not description:
        description = f"Task-local skill `{skill_name}`."
    allowed_tools = _normalize_allowed_tools(meta) or _normalize_allowed_tools(inherited_meta)
    canonical_meta = {
        "name": str(meta.get("name", "") or inherited_meta.get("name", "") or skill_name),
        "description": description,
    }
    if allowed_tools:
        canonical_meta["allowed-tools"] = allowed_tools
    compatibility = str(meta.get("compatibility", "") or inherited_meta.get("compatibility", "")).strip()
    if compatibility:
        canonical_meta["compatibility"] = compatibility
    metadata = meta.get("metadata", {}) or inherited_meta.get("metadata", {})
...
def _augment_skill_markdown(
    ...
    *,
    ...
    skill_md = _normalize_skill_markdown(skill_name, skill_md, files_to_write, existing_meta=existing_meta)
    try:
...
def write_skill_bundle(skill_library_root: Path, skill_name: str, files_to_write: dict[str, str]) -> Path:
    ...
    if existing_skill_md.exists():
        try:
            existing_meta, _ = _split_frontmatter(read_text(existing_skill_md))
        except Exception:
            existing_meta = None
    if "SKILL.md" in normalized_files:
        normalized_files["SKILL.md"] = _augment_skill_markdown(
            skill_name,
            normalized_files["SKILL.md"],
            normalized_files,
